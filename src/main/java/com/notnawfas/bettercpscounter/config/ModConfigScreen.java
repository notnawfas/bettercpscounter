...
public class ModConfigScreen extends Screen {
    ...
    private void handleExpandedCardClick(int mx, int my, int cardX, int cardBodyY, int cardW, int index, CounterEntry entry, ModConfig config) {
        ...
        if (mx >= cx && mx < cx + halfW && my >= cy && my < cy + SWATCH_H) {
            net.minecraft.client.MinecraftClient.getInstance().setScreen(new ColorPickerScreen(this, "Background Color", entry.backgroundColor, c -> {
        ...
        if (mx >= cx + halfW + 8 && mx < cx + halfW + 8 + halfW && my >= cy && my < cy + SWATCH_H) {
            net.minecraft.client.MinecraftClient.getInstance().setScreen(new ColorPickerScreen(this, "Text Color", entry.textColor, c -> {
                entry.textColor = c;
                ModConfig.getConfig().requestSave();
            }));
            playClickSound();
        ...
        if (isCustom && mx >= editBtnX && mx < editBtnX + editBtnW && my >= cy && my < cy + EDIT_BTN_H) {
            net.minecraft.client.MinecraftClient.getInstance().setScreen(new HudPositionScreen(this, entry));
            playClickSound();
            return;
        }
        cy += EDIT_BTN_H + WIDGET_SPACING + FIELD_GAP;
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
        if (sliderDragging && sliderCardIndex >= 0 && sliderCardIndex < ModConfig.getConfig().counters.size()) {
            ...
            double ratio = Math.max(0, Math.min(1, (mouseX - cx) / cw));
            double newVal = Math.round((0.5 + ratio * 1.5) / 0.1) * 0.1;
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (focusedCardIndex >= 0 && focusedCardIndex < ModConfig.getConfig().counters.size()
         && focusedCardIndex < cardFieldTexts.size() && focusedCardIndex < nameFieldTexts.size()) {
            if (focusedFieldIndex == 0) { if (editTextField(keyCode, cardFieldTexts, cardFieldCursors, t -> ModConfig.getConfig().counters.get(focusedCardIndex).displayFormat = t)) return true; }
            else if (focusedFieldIndex == 1) { if (editTextField(keyCode, nameFieldTexts, nameFieldCursors, t -> ModConfig.getConfig().counters.get(focusedCardIndex).name = t)) return true; }
        } else {
            focusedCardIndex = -1;
            focusedFieldIndex = -1;
        }
    ...
    public boolean charTyped(char chr, int modifiers) {
        if (focusedCardIndex >= 0 && focusedCardIndex < ModConfig.getConfig().counters.size()
         && focusedCardIndex < cardFieldTexts.size() && focusedCardIndex < nameFieldTexts.size()) {
            if (focusedFieldIndex == 0) { if (charTypedField(chr, cardFieldTexts, cardFieldCursors, t -> ModConfig.getConfig().counters.get(focusedCardIndex).displayFormat = t)) return true; }
            else if (focusedFieldIndex == 1) { if (charTypedField(chr, nameFieldTexts, nameFieldCursors, t -> ModConfig.getConfig().counters.get(focusedCardIndex).name = t)) return true; }
        } else {
            focusedCardIndex = -1;
            focusedFieldIndex = -1;
        }
